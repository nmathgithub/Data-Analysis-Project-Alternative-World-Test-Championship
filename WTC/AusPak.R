...
winner = res[[3]][[16]]
home = res[[1]][[16]]; #home = gsub(" ", "", home, fixed = TRUE); #Removes spaces in Sri Lanka, New Zealand, South Africa, West Indies, etc.
away = res[[2]][[16]]; #away = gsub(" ", "", away, fixed = TRUE); 
# 14,16 for Pakistan-Australia  
...
#1st Test 
#scorecard = readLines("https://espncricinfo.com/ci/engine/match/1183530.html")
#2nd Test 1183531
scorecard = readLines("https://espncricinfo.com/ci/engine/match/1183531.html")
...
#Lunch Data
out1 = grep('Tea:', scorecard) # out1 = grep('Lunch:', scorecard)
# Output: 158 
...
for(x in 1:(0.5*length(lunch0[[1]]))){
 ...
 # lunch11 = gsub('-', '\\1', substring(scorecard[out1],lunch0[[1]][[x]] +7, lunch0[[1]][[x]]+32)) -NORMAL 
  lunch11 = gsub('-', '\\1', substring(scorecard[out1],lunch0[[1]][[x]] +4, lunch0[[1]][[x]]+32)) - D/N
...
#Tea Data
out2 = grep('Dinner:', scorecard) #out2 = grep('Tea:', scorecard)
tea0 = gregexpr('Dinner:', scorecard[out2]) #tea0 = gregexpr('Tea:', scorecard[out2])
tea = rep(NULL)
...
if(anyNA(as.numeric(end[[4]]))==TRUE)
...
} #Inn Break 1st Innings 1st Test data missing.
# inn = prepend(inn, 86.2); inn = prepend(inn, 10); inn = prepend(inn, 240); inn = prepend(inn, 'Pakistan');
#Inn Break 2nd Test 2nd innings data missing (declare)
decl = c('Australia', 589, 3, 127);
testinn = as.data.frame(matrix(inn, ncol=4, byrow = TRUE))
names(testinn) = c('Team', 'Runs', 'Wickets', 'Overs')
testinn = rbind(decl, testinn[1,], testinn[2,]) #Needed for 1st  tests
print(testinn)
...
#testinn[length(testinn[[1]])+1,] = testfinn[1,] #Normal circumstances 
testfinn[[1]] = away;
testinn[length(testinn[[1]]),] = testfinn[1,] #In Case of Declaration in 2nd innings.
print(testfinn)
...
session_data <- function(day, sess_start, sess_end, innings) {
  ...
  if(Swickets < 0){  # index = c(1) # For counter 
    ...
    if(sess_start == 0){
      ...
      SRR1 = Sruns1/Sovers1; print(SRR1)
