...
winner = res[[3]][[56]]
home = res[[1]][[56]]; #home = gsub(" ", "", home, fixed = TRUE); #Removes spaces in Sri Lanka, New Zealand, South Africa, West Indies, etc.
away = res[[2]][[56]]; #away = gsub(" ", "", away, fixed = TRUE); 
...
#1st Test North Sound
#scorecard = readLines("https://espncricinfo.com/ci/engine/match/1252073.html")
#2nd Test North Sound
scorecard = readLines("https://espncricinfo.com/ci/engine/match/1252074.html")
...
result0 = gregexpr('won', scorecard[out0])
print(substring(scorecard[out0], result0[[1]][[1]]-15, result0[[1]][[1]]+15))
#Check if match is day night 
dn = grep('D/N', scorecard)
dn0 = gregexpr('D/N', scorecard[dn])

#Lunch Data 
if(is_empty(dn0) == TRUE)
{out1 = grep('Lunch:', scorecard)
lunch0 = gregexpr('Lunch:', scorecard[out1])
} else # 3RD TEST D/N MATCH
{ out1 = grep('Tea:', scorecard)
lunch0 = gregexpr('Tea:', scorecard[out1]) 
}
...
for(x in 1:(0.5*length(lunch0[[1]]))){
  ...
  lunch11 = gsub('-', '\\1', substring(scorecard[out1],lunch0[[1]][[x]] + 7, lunch0[[1]][[x]]+34)) #+32 1st Test, +34 2nd Test
...
for(i in 1: length(lunch)){
  if((i%%4 == 0) && (anyNA(as.numeric(lunch[[i]]))==TRUE))
  {
...
if(is_empty(dn0) == TRUE)
{
  ...
  out2 = grep('Dinner:', scorecard)
  tea0 = gregexpr('Dinner:', scorecard[out2])
}
#Throw out invalid indices 
inv = grep('Match days', scorecard)
inv0 = gregexpr('Match days', scorecard[inv])
if(tea0[[1]][[1]] <= inv0[[1]][[1]])
{
  tea0[[1]] = tea0[[1]][2:length(tea0[[1]])]
}
...
for(x in 1:(0.5*length(end0[[1]]))){
  ...
  end11 = gsub('-', '\\1', substring(scorecard[out3], end0[[1]][[x]] + 11, end0[[1]][[x]]+40))  # +38 1st test, +40 2nd Test
...
for(i in 1: length(tea)){
  if((i%%4 == 0) && (anyNA(as.numeric(end[[i]]))==TRUE))
  {
...
for(x in 1:(0.5*length(inn0[[1]]))){
  ...
  # end[[x]] = substring(scorecard[out3],end0[[1]][[x]]+11, end0[[1]][[x]]+36 )
  inn11 = gsub('-', '\\1', substring(scorecard[out4], inn0[[1]][[x]] + 15, inn0[[1]][[x]]+42))
...
# 1ST TEST 
# testinn = rbind(testend[2,], testinn)
# 2ND TEST (Day 2 lunch -> 1st Innings Break, 2nd innings break data missing)
err1 = c('Lanka', 258, 10, 107)
testinn = rbind(testlunch[2,], err1, testinn)
...
for(x in 1:(0.5*length(final[[1]]))){
  ...
  # end[[x]] = substring(scorecard[out3],end0[[1]][[x]]+11, end0[[1]][[x]]+36 )
  fin11 = gsub('-', '\\1', substring(scorecard[out5], final[[1]][[x]] - 20, final[[1]][[x]] - 4))
  ...
  fin11 = gsub('n', '\\1', fin11) #3RD TEST INDIA-ENGLAND D/N
...
for(i in 1:days){
  for(j in 0:2){
    ...
    if((list1[[4]]==0 || list1[[5]] >= 4 && list1[[2]] >=4 )) # Stokes, Pant, Myers-Bonner
    {
      bat = 1; batrec = 'T';
      bowl = 1; bowlrec = 'T';
