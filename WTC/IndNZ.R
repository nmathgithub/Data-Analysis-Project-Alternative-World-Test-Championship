...
winner = res[[3]][[28]]
home = res[[1]][[28]]; #home = gsub(" ", "", home, fixed = TRUE); #Removes spaces in Sri Lanka, New Zealand, South Africa, West Indies, etc.
away = res[[2]][[28]]; #away = gsub(" ", "", away, fixed = TRUE); 
# 27-28 for Ind-NZ
...
#1st Test 
# scorecard = readLines("https://espncricinfo.com/ci/engine/match/1187685.html")
#2nd Test 1187686
scorecard = readLines("https://espncricinfo.com/ci/engine/match/1187686.html")
...
# testinn = rbind(testinn[1,], testlunch[3,], testinn[2,]) #1st Test 
missingdata = c('India', 124, 10, 46)
testinn = rbind(testinn[1:2,], missingdata)  #2nd Test 
print(testinn)
...
for(x in 1:(0.5*length(final[[1]]))){
  ...
  # end[[x]] = substring(scorecard[out3],end0[[1]][[x]]+11, end0[[1]][[x]]+36 )
  fin11 = gsub('-', '\\1', substring(scorecard[out5], final[[1]][[x]] - 16, final[[1]][[x]] - 4))
