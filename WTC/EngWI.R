...
winner = res[[3]][[31]]
home = res[[1]][[31]]; #home = gsub(" ", "", home, fixed = TRUE); #Removes spaces in Sri Lanka, New Zealand, South Africa, West Indies, etc.
away = res[[2]][[31]]; #away = gsub(" ", "", away, fixed = TRUE); 
# 20-31 for Eng-WI
...
#1st Test 
#scorecard = readLines("https://espncricinfo.com/ci/engine/match/1225247.html")
#2nd Test 
#scorecard = readLines("https://espncricinfo.com/ci/engine/match/1225248.html")
#3rd Test 1225249
scorecard = readLines("https://espncricinfo.com/ci/engine/match/1225249.html")
...
if(anyNA(as.numeric(tea[[4]]))==TRUE)
{
  tea[[4]] = 0 
}
...
# 2nd Test - Day 3 Washout 
testlunch = rbind(testlunch[1:2,], testend[2,], testlunch[3:4,])
testtea = rbind(testtea[1:2,], testend[2,], testtea[3:4,])
...
for(x in 1:(0.5*length(inn0[[1]]))){
  ...
  # end[[x]] = substring(scorecard[out3],end0[[1]][[x]]+11, end0[[1]][[x]]+36 )
  inn11 = gsub('-', '\\1', substring(scorecard[out4], inn0[[1]][[x]] + 15, inn0[[1]][[x]]+43))
...
# missingdata = c('England', 204, 10, 67.3)
# testinn = rbind(testinn[1:2,], missingdata)  #1st Test 
decl = c('England', 469, 9, 162)
decl2 = c('England', 129, 3, 19)
testinn = rbind(decl,testinn[1,], decl2)  #2nd Test
# missingdata = c('England', 369, 10, 111.5)
# decl = c('England', 226, 2, 58)
# testinn = rbind(missingdata,testinn[1,], decl)  #3rd Test  
#EndofMatch
...
for(x in 1:(0.5*length(final[[1]]))){
  ...
  fin11 = gsub('overs', '\\1', fin11)
  fin11 = gsub('over', '\\1', fin11)
  fin11 = gsub('ove', '\\1', fin11)
  fin11 = gsub('ers', '\\1', fin11)
  fin11 = gsub('ov', '\\1', fin11)
  fin11 = gsub('er', '\\1', fin11)
  fin11 = gsub('o', '\\1', fin11)
  fin11 = gsub('v', '\\1', fin11)
  fin11 = gsub('e', '\\1', fin11)
  # fin11 = gsub('n', '\\1', fin11)
...
session_data <- function(day, sess_start, sess_end, innings) {
  ...
  if(Swickets < 0 || Sovers < 0){  # index = c(1) # For counter 
    # print(session[[4]])
